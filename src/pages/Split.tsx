...
export default function Split() {
  ...
  const handleSetCount = () => {
    ...
    if (!n || n < 2) return
    setCount(n)
    setParticipants(Array.from({ length: n }, (_, i) => ({ id: i + 1, name: "", custom: "" })))
  }

  const updateName = (id: number, name: string) => setParticipants(p => p.map(x => x.id === id ? { ...x, name } : x))
  const updateCustom = (id: number, custom: string) => setParticipants(p => p.map(x => x.id === id ? { ...x, custom } : x))
  const resetCount = () => { setCount(null); setCountInput(""); setParticipants([]) }

  const shareWhatsApp = () => {
    if (!total) return
    let message = "*Split Summary*\nTotal Bill: Rs." + total
    if (parseFloat(tip) > 0) message += "\nTip (" + tip + "%): Rs." + tipAmount.toFixed(2)
    message += "\nGrand Total: Rs." + grandTotal.toFixed(2) + "\n\n"
    if (splitMode === "equal") {
      message += "Each person pays: Rs." + perPerson.toFixed(2) + "\n\n"
      participants.forEach(p => { if (p.name) message += "- " + p.name + ": Rs." + perPerson.toFixed(2) + "\n" })
    } else {
      participants.forEach(p => { if (p.name) message += "- " + p.name + ": Rs." + parseFloat(p.custom || "0").toFixed(2) + "\n" })
    }
    window.open("https://wa.me/?text=" + encodeURIComponent(message), "_blank")
  }

  const shareNative = () => { if (navigator.share) navigator.share({ title: "Split Summary", text: "Total: Rs." + grandTotal.toFixed(2) + " | Per person: Rs." + perPerson.toFixed(2) }) }
      ...
      <div className="p-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4 space-y-4">
          <div>
            ...
            <input type="number" placeholder="0.00" value={total} onChange={e => setTotal(e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-400" />
          </div>
          <div>
        ...
        {!count ? (
          <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4">
            ...
            <div className="flex gap-2 mb-3">
              {[2, 3, 4, 5, 6].map(n => (
                <button key={n} onClick={() => setCountInput(String(n))} className={"flex-1 py-2 rounded-xl text-sm font-medium border " + (countInput === String(n) ? "bg-violet-600 text-white border-violet-600" : "border-gray-200 text-gray-500")}>{n}</button>
              ))}
            </div>
            <div className="flex gap-2">
              <input type="number" placeholder="Custom number" value={countInput} onChange={e => setCountInput(e.target.value)} className="flex-1 border border-gray-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-violet-400" />
              <button onClick={handleSetCount} className="bg-violet-600 text-white rounded-xl px-4 py-2 text-sm font-medium">Continue</button>
            </div>
          </div>
        ) : (
          <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4">
            ...
            <div className="space-y-2">
              {participants.map((p, i) => (
                <div key={p.id} className="flex items-center gap-2">
                  <div className="w-7 h-7 bg-violet-100 rounded-full flex items-center justify-center text-violet-600 text-xs font-bold flex-shrink-0">{i + 1}</div>
                  <input type="text" placeholder={"Person " + (i + 1) + " name"} value={p.name} onChange={e => updateName(p.id, e.target.value)} className="flex-1 border border-gray-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-violet-400" />
                  {splitMode === "unequal" && <input type="number" placeholder="Rs." value={p.custom} onChange={e => updateCustom(p.id, e.target.value)} className="w-20 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-violet-400" />}
                </div>
              ))}
            </div>
          </div>
        ...
        {splitMode === "unequal" && total && count && Math.abs(unequalDiff) > 0.01 && (
          <div className={"rounded-xl px-4 py-2 text-sm mb-4 " + (unequalDiff > 0 ? "bg-orange-50 text-orange-500" : "bg-red-50 text-red-500")}>
            {unequalDiff > 0 ? "Rs." + unequalDiff.toFixed(2) + " still unassigned" : "Rs." + Math.abs(unequalDiff).toFixed(2) + " over the total"}
          </div>
        )}

        {total && count && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-gray-900 rounded-2xl p-5 text-white mb-4 relative overflow-hidden">
            ...
            <div className="relative">
              {splitMode === "equal" ? (
                <>
                  ...
                  <h2 className="text-4xl font-bold">Rs.{perPerson.toFixed(2)}</h2>
                  <p className="text-gray-400 text-sm mt-2">Rs.{grandTotal.toFixed(2)} divided by {count} people</p>
                </>
              ) : (
                <>
                  ...
                  {participants.map(p => (
                    <div key={p.id} className="flex justify-between text-sm py-1">
                      ...
                      <span className="font-semibold">Rs.{parseFloat(p.custom || "0").toFixed(2)}</span>
                    </div>
                  ))}
                </>
              )}
