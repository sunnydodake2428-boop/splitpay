...
import { Plus, Trash2, MessageCircle, Share2, Download } from 'lucide-react'
...
export default function Split() {
  ...
  const [participants, setParticipants] = useState<Participant[]>([{ id: 1, name: '', custom: '' }])
  ...
  const addParticipant = () => setParticipants(p => [...p, { id: Date.now(), name: '', custom: '' }])
  const removeParticipant = (id: number) => { if (participants.length === 1) return; setParticipants(p => p.filter(x => x.id !== id)) }
  ...
  return (
    <div>
      ...
      <div className="p-4">
        <BillScanner onAmountDetected={(amt) => setTotal(amt)} />
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4">
          <p className="text-sm text-gray-500 mb-3">Participants ({participants.length})</p>
          <div className="space-y-2">
            {participants.map((p, i) => (
              <div key={p.id} className="flex items-center gap-2">
                <input type="text" placeholder={`Person ${i + 1}`} value={p.name} onChange={e => updateName(p.id, e.target.value)} className="flex-1 border border-gray-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-violet-400" />
                {splitMode === 'unequal' && <input type="number" placeholder="₹" value={p.custom} onChange={e => updateCustom(p.id, e.target.value)} className="w-20 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-violet-400" />}
                <button onClick={() => removeParticipant(p.id)} className="text-red-400 p-2"><Trash2 size={16} /></button>
              </div>
            ))}
          </div>
          <button onClick={addParticipant} className="mt-3 flex items-center gap-2 text-violet-600 text-sm font-medium"><Plus size={16} /> Add Person</button>
        </div>

        {splitMode === 'unequal' && total && Math.abs(unequalDiff) > 0.01 && (
            ...
            <div className="relative">
              {splitMode === 'equal' ? (
                <>
                  ...
                  <p className="text-gray-400 text-sm mt-2">₹{grandTotal.toFixed(2)} ÷ {participants.length} people</p>
                </>
              ) : (
                <>
