...
export default function Home() {
  ...
  return (
    <motion.div className="min-h-screen bg-gray-50" initial="hidden" animate="visible" variants={containerVariants}>
      <motion.div variants={itemVariants} className="bg-white px-5 pt-8 pb-5 border-b border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <motion.div whileTap={{ scale: 0.95 }} onClick={() => navigate('/profile')} className="cursor-pointer">
            <img src={localStorage.getItem('splitpay_avatar') || user?.imageUrl} alt="avatar" className="w-10 h-10 rounded-full object-cover ring-2 ring-violet-100" />
          </motion.div>
        </div>
      </motion.div>

      <div className="p-5 space-y-6">
        <motion.div variants={itemVariants} whileHover={{ scale: 1.01 }} className="relative bg-gray-900 rounded-3xl p-6 text-white overflow-hidden cursor-pointer" onClick={() => navigate('/analytics')}>
          ...
          <div className="relative">
            ...
            <motion.h2 initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3, duration: 0.5 }} className="text-4xl font-bold tracking-tight mb-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 text-xs text-gray-400">View analytics <ArrowRight size={11} /></div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants}>
          ...
          <div className="grid grid-cols-4 gap-3">
            {actions.map(({ icon: Icon, label, color, path }, i) => (
              <motion.button key={label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.06 }} whileTap={{ scale: 0.92 }} onClick={() => navigate(path)} className="flex flex-col items-center gap-2">
        ...
        <motion.div variants={itemVariants}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate('/groups')} className="text-xs text-violet-500 font-medium flex items-center gap-1">See all <ArrowRight size={11} /></button>
          </div>
          {recentGroups.length === 0 ? (
            <motion.div whileTap={{ scale: 0.98 }} onClick={() => navigate('/groups')} className="bg-white rounded-2xl p-6 border border-dashed border-gray-200 text-center cursor-pointer">
            ...
            <div className="space-y-2">
              {recentGroups.map((g, i) => {
                ...
                return (
                  <motion.div key={g.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.08 }} whileTap={{ scale: 0.98 }} onClick={() => navigate('/groups/' + g.name)} className="bg-white rounded-2xl p-4 border border-gray-100 flex items-center justify-between cursor-pointer">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold text-gray-800">Rs.{total.toFixed(2)}</p>
        ...
        <motion.button variants={itemVariants} whileTap={{ scale: 0.97 }} onClick={() => navigate('/split')} className="w-full bg-gray-900 text-white rounded-2xl py-4 font-semibold text-sm flex items-center justify-center gap-2 tracking-wide">
